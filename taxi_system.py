...
class Taxi_system():
    ...
    def oper_ride(option, username):    #losowanie poczatkowej lokalizacji, wybieranie punktu destynacji/ pobieanie pieniedzy z portfela
        def file_oper():
            with open(f'txt/{username}_wallet.txt', 'r') as l:    
                    line = l.read()
            with open(f'txt/{username}_wallet.txt', 'w') as k:
                if float(line) != 0:
                    ...
                    print("!! No funds avaible.\nPlease deposit $ to your wallet to finish the transaction.\n")
                elif int(line) == 0:
                    print("!! No funds avaible.\nPlease deposit $ to your wallet to finish the transaction.\n")
